...
export const getColorIcon = (props) => {
  if (props.disabled) {
...
export const InnerLabel = styled.div`
  ...
  margin-right: 8px;
`;
export const InnerLabelMutiple = styled(InnerLabel)`
  color: ${({ selectedValue }) => (selectedValue ? '#262626' : '#9d9d9d')};
`;
...
export const OptionsItem = styled.div`
  ...
  height: ${getHeight};
  padding: 11px 24px;
  color: ${({ color }) => (color ? `${color.text}` : '#9D9D9D')};
  box-sizing: border-box;
  transition: 0.2s ease;
  /* background: ${({ active }) => (active ? '#E9E9E9' : 'unset')}; */
  background: ${({ color }) => (color ? `${color.bg}` : 'unset')};
  cursor: pointer;
...
export const Select = styled.div`
  ...
  background-color: ${({ disabled }) =>
    disabled ? '#ffffff' : getBackgroundColor};
  color: ${getColor};
